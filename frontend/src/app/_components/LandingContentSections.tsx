        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {features.map((f) => (
            ...
            >
              ...
              <div className="mt-4 text-base font-bold text-slate-900">{f.title}</div>
              <div className="mt-2 text-sm text-slate-700 leading-relaxed">{f.desc}</div>
            </div>
          ))}
        </div>

        <div className="mt-14 rounded-xl border border-slate-200 bg-white p-8 shadow-sm">
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
            <div>
              <h3 className="text-xl font-semibold text-slate-900">Start earning in 3 simple steps</h3>
              ...
              <div className="mt-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  ...
                  <div key={s.n} className="rounded-xl bg-slate-50 border border-slate-200 p-4">
                    <div className="text-xs font-bold text-slate-500">{s.n}</div>
                    <div className="mt-1 font-bold text-slate-900">{s.t}</div>
                    <div className="mt-1 text-sm text-slate-700">{s.d}</div>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="w-full lg:w-auto">
              <Link
                prefetch={false}
                href="/register"
                className="inline-flex w-full lg:w-auto items-center justify-center gap-2 rounded-xl px-5 py-3 text-sm font-semibold text-white shadow-sm bg-gradient-to-r from-emerald-600 via-teal-600 to-sky-600 hover:opacity-95 transition"
              >
                Get Started <ArrowRight className="h-4 w-4" />
              </Link>
              <div className="mt-2 text-xs text-slate-600">
                Already have an account?{" "}
                <Link prefetch={false} href="/login" className="font-semibold text-teal-700 hover:underline">
                  Sign in
                </Link>
              </div>
            </div>
