...
export default function AdminServicesPage() {
  ...
  const stats = useMemo(() => {
    ...
    const pending = services.filter((s) =>
      s.status === "draft" || s.status === "pending" || s.status === "pending_approval"
    ).length;
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              {stats.pending > 0 && (
                <Link
                  href="/dashboard/admin/service-approval"
                  className="inline-flex items-center gap-1.5 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-1 text-xs font-semibold text-amber-800 transition hover:bg-amber-100"
                >
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">
                    ...
                    <input
                      ...
                      className="hidden"
                      disabled={!canManage || imageUploading}
                      onChange={async (e) => {
                        const f = e.target.files?.[0];
                        if (!f) return;
                        e.target.value = "";
                        const url = await uploadServiceImage(f, false);
                        if (url) setImage(url);
                      }}
                    />
                  </label>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">
                    ...
                    <input
                      ...
                      accept="image/jpeg,image/jpg,image/png,image/gif,image/webp"
                      className="hidden"
                      disabled={!canManage || editImageUploading}
                      onChange={async (e) => {
                        const f = e.target.files?.[0];
                        if (!f) return;
                        e.target.value = "";
                        const url = await uploadServiceImage(f, true);
