...
import { formatActionLabel, formatINRPrecise, personLabel } from "../_lib";
...
export default function IncomeReportsAuditPage() {
  ...
  const load = useCallback(async () => {
    setBusy(true);
    try {
      const res = await apiFetch("/api/admin/reports/payment-audit?limit=200");
  ...
  const fileBase = `customers-income-audit-${new Date().toISOString().slice(0, 10)}`;

  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <p className="text-sm text-zinc-600">
          Admin actions on withdrawals and manual payouts (who changed what and when).
        </p>
        <ReportExportButtons
          reportTitle="Customers Income — Audit Log"
