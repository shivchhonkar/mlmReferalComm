...
export default function IncomeReportsHistoryPage() {
  ...
  const load = useCallback(async () => {
    setBusy(true);
    try {
      const qs = new URLSearchParams({ status, limit: "200", scope: "mine" });
  ...
  const fileBase = `admin-income-history-${status}-${new Date().toISOString().slice(0, 10)}`;

  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm sm:p-6">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <p className="text-sm text-zinc-600">
          Withdrawals you reviewed or paid. UPI rows include screenshot proof for audit.
        </p>
        <ReportExportButtons
          reportTitle="Admin Income — Payment History"
            ...
            {rows.map((w) => (
              <tr key={w._id} className="border-b border-zinc-100">
                <td className="px-3 py-3">
                  ...
                  {w.user?._id ? (
                    <Link
                      href={`/dashboard/admin/reports/income-reports/${w.user._id}`}
