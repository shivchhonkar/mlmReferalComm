    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-emerald-50/60 via-white to-zinc-50">
        ...
        <div className="mx-auto max-w-2xl px-6 py-12">
          <h1 className="text-2xl sm:text-xl  tracking-tight text-zinc-900">
            Dashboard
          </h1>
          <div className="mt-5 rounded-2xl border border-red-200 bg-white p-6 text-sm text-red-700 shadow-sm">
            {error} —{" "}
            <Link prefetch={false} className="  underline hover:text-red-800" href="/login">
              Login
            </Link>
          </div>
        </div>
        ...
        <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            {/* <div className="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-white/70 px-3 py-1.5 shadow-sm backdrop-blur">
              ...
              <span className="text-xs sm:text-sm  text-zinc-800">
            ...
            <h1 className="mt-0 text-sm sm:text-lg  tracking-tight text-zinc-900">
              Dashboard
            </h1>
            ...
            {me ? (
              <p className="mt-2 text-sm sm:text-base text-zinc-700">
                Welcome back,{" "}
                <span className="  bg-gradient-to-r from-emerald-700 to-sky-700 bg-clip-text text-transparent">
                  {me.email}
                </span>{" "}
                <span className="text-zinc-500 text-sm">({me.role})</span>
              </p>
            ) : null}
          </div>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            {["super_admin", "admin", "moderator"].includes(me?.role || "") ? (
              <Link
                className="inline-flex items-center gap-2 rounded-2xl border border-emerald-200 bg-white px-4 py-2.5 text-sm   text-emerald-700 shadow-sm transition hover:bg-emerald-50"
                prefetch={false}
                href="/admin"
              >
            ...
            <Link
              className="inline-flex items-center gap-2 rounded-2xl border border-zinc-200 bg-white px-4 py-2.5 text-sm   text-zinc-800 shadow-sm transition hover:bg-zinc-50"
              prefetch={false}
              href="/cart"
            >
          ...
          <div className="mb-8 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base sm:text-lg  text-zinc-900">
                    Team Size
                  </h2>
                  <p className="text-xs sm:text-sm text-zinc-600">
                    Your downline growth snapshot
                  </p>
                </div>
              </div>
            ...
            <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
              ...
              <p className="mt-2  text-xl sm:text-xl  tracking-tight bg-gradient-to-r from-emerald-700 to-sky-700 bg-clip-text text-transparent">
                {formatINR(totalIncome)}
              </p>

              <p className="mt-2 text-xs sm:text-sm text-zinc-600">
                Cumulative income from your network.
              </p>
            </div>
            ...
            <div className="rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md md:col-span-2 lg:col-span-1">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm sm:text-base">
                  ...
                  <span className=" text-zinc-900">{incomes.length}</span>
                </div>
                <div className="flex justify-between text-sm sm:text-base">
                  <span className="text-zinc-600">Available Services</span>
                  <span className=" text-zinc-900">{services.length}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* <div className="rounded-3xl border border-emerald-200 bg-white p-6 shadow-sm transition hover:shadow-md">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-zinc-600">
                Choose a service and add it to cart or buy instantly.
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center">
            <select
              className="flex-1 rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm sm:text-base   text-zinc-900 shadow-sm transition focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400"
            ...
            <div className="flex gap-2 sm:flex-col">
              <button
                className="rounded-2xl border border-emerald-200 bg-white px-6 py-3 text-sm sm:text-base  text-emerald-800 shadow-sm transition hover:bg-emerald-50 disabled:opacity-60"
              ...
              <button
                className="rounded-2xl bg-gradient-to-r from-emerald-600 to-sky-600 px-6 py-3 text-sm sm:text-base  text-white shadow-lg transition hover:from-emerald-700 hover:to-sky-700 hover:shadow-xl disabled:opacity-60"
                onClick={buyService}
                disabled={busy || !selectedServiceId}
                type="button"
              >
        ...
        <div className="mt-6 rounded-3xl border border-emerald-200 bg-white p-6 shadow-sm transition hover:shadow-md">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-zinc-600">
                  Track your referral earnings over time.
                </p>
              </div>
            </div>

            <Link
              className="text-sm   bg-gradient-to-r from-emerald-700 to-sky-700 bg-clip-text text-transparent hover:underline"
              prefetch={false}
              href="/referrals"
            >
              View Referral Tree →
          ...
          <div className="mt-4 overflow-auto rounded-2xl border border-zinc-200">
            <table className="w-full text-sm ">
              ...
              <tbody>
                {incomes.map((inc) => (
                  <tr key={inc._id} className="border-t border-zinc-200 hover:bg-emerald-50/40 transition-colors">
                    ...
                    <td className="py-3 px-4   text-zinc-900">{inc.bv}</td>
                ...
                {incomes.length === 0 ? (
                  <tr>
                    <td className="py-10 text-center text-zinc-600" colSpan={5}>
