...
export default function AdminServiceUpload() {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const response = await apiFetch("/api/admin/services/template/download");
      ...
      link.href = url;
      link.download = "service-import-template.xlsx";
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await apiFetch("/api/admin/services/bulk-upload", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Bulk Service Import</h2>
        <p className="text-gray-600">Upload Excel or CSV files to import multiple services at once</p>
      </div>
      ...
      {result && result.success && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-6">
          <div className="flex gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-green-700 mt-1">
                {result.successfulInserts} out of {result.totalRows} services imported
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
            ...
            {/* By Status */}
            {Object.keys(result.summary.byStatus).length > 0 && (
              <div className="bg-white rounded p-4 border border-green-100">
                <h4 className="font-medium text-gray-900 mb-3">By Status</h4>
                <div className="space-y-2">
                  {Object.entries(result.summary.byStatus).map(([status, count]) => (
      ...
      {result && !result.success && result.successfulInserts > 0 && (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
          <div className="flex gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700 mt-1">
                {result.successfulInserts} out of {result.totalRows} services imported successfully
              </p>
            </div>
          </div>
          ...
          {/* Error Details */}
          {result.errors && result.errors.length > 0 && (
            <div className="mt-4 bg-red-50 rounded p-4 border border-red-100">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        ...
        <ul className="space-y-2 text-sm text-gray-700">
          <li className="flex items-start gap-3">
            ...
            <span><strong>Required fields:</strong> name, slug, image, price, businessVolume</span>
          </li>
          <li className="flex items-start gap-3">
            <span className="text-blue-600 font-bold">•</span>
            <span><strong>Optional fields:</strong> originalPrice, currency, discountPercent, shortDescription, description, status, isFeatured, categoryCode, tags, gallery</span>
          </li>
          <li className="flex items-start gap-3">
            <span className="text-blue-600 font-bold">•</span>
            <span><strong>categoryCode:</strong> Use the category code (e.g., FIN, TAX) to automatically assign services to categories</span>
          </li>
          <li className="flex items-start gap-3">
            <span className="text-blue-600 font-bold">•</span>
            <span><strong>tags & gallery:</strong> Use comma-separated values (e.g., "tag1,tag2,tag3")</span>
          </li>
          <li className="flex items-start gap-3">
            <span className="text-blue-600 font-bold">•</span>
            <span><strong>isFeatured:</strong> Use true/false, TRUE/FALSE, or 1/0</span>
          </li>
          <li className="flex items-start gap-3">
            <span className="text-blue-600 font-bold">•</span>
            <span><strong>status:</strong> active, inactive, or out_of_stock</span>
          </li>
        </ul>
      </div>
    </div>
