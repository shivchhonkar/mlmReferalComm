  ...
  try {
    ...
    pipeline.push({
      $facet: {
        ...
        data: [
          ...
          // Compute direct business volume (BV) generated by this user
          {
            $lookup: {
              from: "purchases",
              let: { userId: "$_id" },
          ...
          },
          {
            $addFields: {
              businessVolume: {
                $ifNull: [{ $first: "$purchaseStats.totalBV" }, 0],
              },
            },
          },
          ...
          // Clean up
          { $project: { parentUser: 0, purchaseStats: 0 } },
        ],
      },
    })
    ...
    return res.json({
      ...
      items: data.map((u: any) => ({
        ...
        businessVolume: typeof u.businessVolume === "number" ? u.businessVolume : 0,
